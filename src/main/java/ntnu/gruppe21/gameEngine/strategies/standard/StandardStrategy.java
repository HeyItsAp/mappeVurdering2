 ...
 * random to calculate the price but also incorporates difficulty and a random boost.
...
public class StandardStrategy implements PriceStrategy {
    ...
    /**
     * boostStat:
     *   A value that can between a maximum of 0.0 - 0.80. correlates to 0% - 80%.
     *   Adds random percent-wise increase in value. A true gamble.
     *   Higher difficulties give stronger increases.
     *   Refer to {@link #setDifficulty(Difficulty)}
     */
     ...
     *
     * <p>
     *     First random boost calculated based on {@code boostStat}.
     *     Random produces a number between -1.0 and 1.0 which then multiplied
     *     by max possible percentage change, then boost is added
    ...
    public void calculateNewPrice(List<Stock> stocks) {
        ...
        for (Stock stock : stocks){
            stock.getSalesPrice().multiply(BigDecimal.valueOf(1 + finalChange)).setScale(2, RoundingMode.HALF_UP);
        }
    }

    /**
     * Difficulty effect changeRate (in procent, 0.2 = 2%). Which means that the stock
     * can be increase/decrease by a maximum of the changeRate.
     * <p>
     *  EASY -> 0.05 (5%)
     *  MEDIUM -> 0.10 (10%)
     *  HARD -> 0.15 (15%)
     ...
     * </p>
     *
     * Difficulty change max BoostStat. Higher difficulties will yield better profit changes:
     * <p>
     *  EASY -> 0.10 (10%)
     *  MEDIUM -> 0.20 (20%)
     *  HARD -> 0.40 (40%)
    ...
    public void setDifficulty(Difficulty difficulty) {
        this.changeRate = switch (difficulty){
            case EASY -> 0.02;
            case MEDIUM -> 0.07;
            case HARD -> 0.15;
            case REALISTIC -> 0.25;
        };
        this.boostStat = switch (difficulty){
            case EASY -> 0.1;
            case MEDIUM -> 0.2;
            case HARD -> 0.40;
            case REALISTIC -> 0.80;
