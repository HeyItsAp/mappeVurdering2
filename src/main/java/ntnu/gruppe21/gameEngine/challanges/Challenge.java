...
/**
 * Wraps the {@link ChallengeType}, gives it a visual description and has functions to
 * check if player has completed the challange, and get advance into a new challange.
 *
 * <p>
 *   Challenges needs to be created after player creation, as players chosen starting money effects
 *   certain challanges.
...
public class Challenge {
    ...
    /** Description, more or less a progressbar. Displays your progress and challange description from {@link ChallengeType} **/
    private String description;

    /**
     * Depends on the {@link ChallengeType}
     * - {@code UNIQUE_SHARE_REQUIREMENT}. Will be low integer that keeps rising
     * - {@code BALANCE_REQUIREMENT}. Based on starting money
     */
     ...
     *
     * @param player certain {@link ChallengeType} depend on starting stats of player, so its requires.
    ...
    public void refreshDescription(Player player, ChallengeType challengeType) {
        description = switch (challengeType){
            case UNIQUE_SHARE_REQUIREMENT ->
                challengeType.getChallengeTitle() + ": " + player.getPortfolio().countDistinctStock() + " / " + targetValue;
            case BALANCE_REQUIREMENT ->
                challengeType.getChallengeTitle() + ": " + player.getCurrentMoney() + " / " + targetValue;
        };
    }

    /**
     * Calculates the INITIAL goal requirements based on difficulty and challenge. Method contains base values
     * for each type that is then modified by {@link Difficulty} to produce the final requirement to
     * complete the challenge.
     *
     * @param challengeType Right now, can be of two types. Different types use different base values and multipliers
    ...
    private int setInitialValue(ChallengeType challengeType, Difficulty difficulty, Player player){
        ...
        try {
            baseBalanceRequirement = player.getCurrentMoney().divide(BigDecimal.TWO, RoundingMode.HALF_UP).intValue();
    ...
    /**
     * Checks if challenge is completed by player by going through values according to {@link ChallengeType}
     *
     * @param player
    ...
    public boolean checkCompletion(Player player){
        ...
        boolean done = switch (challengeType){
            case BALANCE_REQUIREMENT ->
    ...
    /**
     * Calculates the A NEW goal requirements based on initial goal. Right now does a simple arithmetic addition
     * to calculate new goal. One of params is {@code newWeek} (which this method doesn't use) but opens for
     * interesting ways to calculate new goals.
     *
     * @param player certain {@link ChallengeType} depend on starting stats of player, so its required
    ...
    public void advanceChallenge(int newWeek, Player player){
        ...
        targetValue = switch (challengeType) {
            case UNIQUE_SHARE_REQUIREMENT -> switch (difficulty) {
                case EASY, MEDIUM, HARD, REALISTIC -> targetValue + 1;
            };
            case BALANCE_REQUIREMENT -> switch (difficulty) {
