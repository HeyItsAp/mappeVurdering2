...
public class Exchange {
    ...
    public List<Stock> findStock(String searchTerm) {
        List<Stock> filtedStocks = stockMap.values().stream()
                .filter(s -> s.getSymbol().toLowerCase().contains(searchTerm.toLowerCase()) || s.getCompany().toLowerCase().contains(searchTerm.toLowerCase()))
                .toList();
        return filtedStocks;
    }
     ...
     * Attempt details: When buying a stock it creates an transaction, this time a purchase. 
     *      This requires to update players portfolio and transaction archive. 
     * 
     * @param symbol
     * @param quantity
     * @param player
    ...
    private void advance(){
        ...
        for (Stock stock : stockMap.values()) {
            ...
            double changePercent = (random.nextDouble() - 0.5) * 0.1; // Simulate a price change between -5% and +5%
