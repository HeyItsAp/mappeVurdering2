/**
 * An archive of transactions, which can be used to store and retrieve transactions based on the week they were made.
 * 
 * @author Adrian
 */
...
public class TransactionArchive {
    ...
    public List<Purchase> getPurchases(int week) {
        List<Purchase> filteredTransactions = transactions.stream()
                .map(t -> (Purchase) t)
                .filter(t -> t.getWeek() == week)
                .toList();
        return filteredTransactions;
    ...
    public List<Sale> getSales(int week) {
        List<Sale> filteredTransactions = transactions.stream()
                .map(t -> (Sale) t)
                .filter(t -> t.getWeek() == week)
                .toList();
        return filteredTransactions;
    ...
    public int countDistinctWeeks(){
        List<Integer> weeks = transactions.stream()
                .map(t -> t.getWeek())
                .distinct()
                .toList();
        return weeks.size();
